...
class PoseControl:
    def __init__(self):
        ...
        self.Kd = rospy.get_param('NS/Kd', 0.1)
        ...
        if self.guardar_datos:
            ...
            directory = os.path.join(package_path,'datos','NS_PID')
            if not os.path.exists(directory):
                os.makedirs(directory)  # Create the directory if it doesnt exist
            self.file_name = os.path.join(directory, "NS_PID_{}.txt".format(path_type))
    ...
    def run(self):
        ...
        for i in range(0,len(self.goalx)):
            ...
            acx_task1 = self.Kp * e_x + self.Ki * ei_x + self.Kd * ed_x
            acy_task1 = self.Kp * e_y + self.Ki * ei_y + self.Kd * ed_y
            acw_task2 = self.Kp * e_theta + self.Ki * ei_theta + self.Kd * ed_theta
